Feature: Tests the Chenile Workflow Service using a REST client. Payment service exists and is under test.
It helps to create a payment and manages the state of the payment as follows:
OPENED -(assign) -> ASSIGNED -(resolve) -> RESOLVED -(close) -> CLOSED
 
  Scenario: Test create payment
	...
	Scenario: Assign the payment to an assignee with comments
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
		When I PATCH a REST request to URL "/payment/${id}/assign" with payload
		"""
		{
	 ...
	 Scenario: Resolve the payment with comments
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
		When I PATCH a REST request to URL "/payment/${id}/resolve" with payload
		"""
		{
	 ...
	 Scenario: Close the payment with comments
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
		When I PATCH a REST request to URL "/payment/${id}/close" with payload
		"""
		{
			"comment": "OK"
		}
...
Scenario: Assign a closed payment to someone. This will err out.
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
		When I PATCH a REST request to URL "/payment/${id}/assign" with payload
		"""
