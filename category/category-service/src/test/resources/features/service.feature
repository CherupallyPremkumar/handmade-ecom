Feature: Tests the category Service using a REST client.
 
  Scenario: Save the category first.
    ...
    And I POST a REST request to URL "/category" with payload
    """
    {
  ...
  Scenario: Retrieve the saved category
    Given that "entity" equals "category"
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
  ...
  Scenario: Save a category using an ID that already is determined
