Feature: Tests the Chenile Workflow Service using a REST client. Tenant service exists and is under test.
It helps to create a tenant and manages the state of the tenant as follows:
OPENED -(assign) -> ASSIGNED -(resolve) -> RESOLVED -(close) -> CLOSED
 
  Scenario: Test create tenant
	...
	Scenario: Assign the tenant to an assignee with comments
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
		When I PATCH a REST request to URL "/tenant/${id}/assign" with payload
		"""
		{
	 ...
	 Scenario: Resolve the tenant with comments
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
		When I PATCH a REST request to URL "/tenant/${id}/resolve" with payload
		"""
		{
	 ...
	 Scenario: Close the tenant with comments
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
		When I PATCH a REST request to URL "/tenant/${id}/close" with payload
		"""
		{
			"comment": "OK"
		}
...
Scenario: Assign a closed tenant to someone. This will err out.
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
		When I PATCH a REST request to URL "/tenant/${id}/assign" with payload
		"""
