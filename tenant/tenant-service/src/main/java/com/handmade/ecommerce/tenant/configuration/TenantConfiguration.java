...
import com.handmade.ecommerce.tenant.model.TenantContext;
...
public class TenantConfiguration {
    ...
    @Bean
    STMFlowStoreImpl tenantFlowStore(@Qualifier("tenantBeanFactoryAdapter") BeanFactoryAdapter tenantBeanFactoryAdapter)
            throws Exception {
    ...
    @Bean
    EntityStore<Tenant> tenantEntityStore() {
        return new TenantEntityStore();
    }

    @Bean
    @Autowired
    StateEntityServiceImpl<Tenant> _tenantStateEntityService_(
    ...
    @Bean
    TenantHealthChecker tenantHealthChecker() {
        return new TenantHealthChecker();
    }

    @Bean
    STMTransitionAction<Tenant> defaulttenantSTMTransitionAction() {
    ...
    @Bean
    @Autowired
    StmBodyTypeSelector tenantBodyTypeSelector(
            @Qualifier("tenantActionsInfoProvider") STMActionsInfoProvider tenantInfoProvider,
    ...
    // Create the specific transition actions here
    @Bean
    ResolveTenantAction tenantResolve() {
        return new ResolveTenantAction();
    }

    @Bean
    CloseTenantAction tenantClose() {
        return new CloseTenantAction();
    }

    @Bean
    AssignTenantAction tenantAssign() {
        return new AssignTenantAction();
    }

    @Bean
    @Autowired
    Function<ChenileExchange, String[]> tenantEventAuthoritiesSupplier(
            @Qualifier("tenantActionsInfoProvider") STMActionsInfoProvider tenantInfoProvider)
            throws Exception {
